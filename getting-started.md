...
- **Tab bar** (top) — open requests and environments as tabs. Each request tab shows the HTTP method badge and name. An orange dot indicates unsaved changes. Drag tabs left or right to reorder them — neighboring tabs slide apart to preview the new position. Right-click a tab for options: Pin, Close, Close Others, Close All. Middle-click to close a tab. Pinned tabs survive "Close All."
